 ...
 * @ Modified time: 2022-08-26 22:33
...
void SSFX_ScreenSpaceReflections(float2 tc, float4 P, float3 N, float gloss, inout float3 color, uint iSample : SV_SAMPLEINDEX)
{
	...
	if (all(hit_uv.xy))
	{
		...
		refl_ray *= saturate(0.25f + (length(refl_ray) < 1.6f));
	...
	// Raise reflection intensity and max limit when raining. ( Thought: Maybe wetness factor? (rain_params.y) )
