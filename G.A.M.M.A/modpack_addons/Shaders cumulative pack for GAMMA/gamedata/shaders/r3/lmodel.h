...
float4 compute_lighting(float3 N, float3 V, float3 L, float4 alb_gloss, float mat_id)
{
...
#ifdef USE_PBR
	float4 light = float4(Lit_BRDF(rough, albedo, f0, V, N, L ), 0);
#else
	float spec = SRGBToLinear(alb_gloss.w*Ldynamic_color.w);
	float4 light = float4(Lit_BRDF(rough, 0, f0, V, N, L ), 0);
	light *= spec;
	...
	if(abs(mat_id-MAT_FLORA) <= 0.05) //Be aware of precision loss/errors
	{
...
float3 specular_phong(float3 pnt, float3 normal, float3 light_direction)
{
	...
	return L_sun_color.rgb * Lit_Specular(nDotL, nDotH, nDotV, lDotH, 0.056, 0.1);
}
